...
**Open Design** opens its installed desktop app on macOS or Windows after starting the proxy. Use **Set up connection** on its card for one-time manual OpenAI BYOK setup. It uses exact IDs from the shared library without importing settings, synchronizing its model catalog or handing off a project folder. See [Open Design setup and Linux guidance](open-design.md).
