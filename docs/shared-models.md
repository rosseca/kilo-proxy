...
When **Maximum** cannot apply to the entire library, the native Models page lists the saved models without a known usable maximum and distinguishes missing catalog entries from entries without a published limit. If the whole catalog is unavailable, it asks you to refresh instead of presenting all saved models as removal candidates. With a loaded catalog you can refresh, or remove one or all listed models from your saved library after reviewing a confirmation. Removing them does not delete a provider model or alter your Kilo account. After removal, **Add replacement models** opens the existing catalog picker; replacements are not chosen automatically, and they also need a known maximum to use **Maximum** for the entire library. Already-open agents retain their existing profiles until reopened.
