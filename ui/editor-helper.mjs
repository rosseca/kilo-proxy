import {claudeDesktopModelSupported,claudeDesktopSelection} from './claude-desktop-helper.mjs';
...
export function createEditorHelper({api,notify,copy,refreshCatalog,onChange=()=>{}}){
 ...
 let ctx={client:'opencode',language:'en',catalog:[]},working=false,signature='',renderedClient='',desktopLibraryLoaded=false,desktopLibraryLoading=false,desktopOmitted=0,desktopFallback=false;
 const L=(en,es)=>ctx.language==='en'?en:es,s=()=>selections[ctx.client];
 const payload=()=>ctx.client==='claude-desktop'?claudeDesktopSelection([...s().models.values()],s().initial).selection:ctx.client==='omp'?ompPayload([...s().models.values()],s().initial):editorPayload([...s().models.values()],s().initial);
 ...
 function controls(){
  ...
  $('editor-load').disabled=working;$('editor-refresh').disabled=working;$('editor-add').disabled=working||desktop&&!claudeDesktopModelSupported($('editor-id').value.trim());
  $('editor-id').placeholder=desktop?'anthropic/claude-sonnet-4.6':'provider/model';
 ...
 function render(context,force=false){
  ...
  const all=new Map(ctx.catalog.filter(m=>ctx.client!=='claude-desktop'||claudeDesktopModelSupported(m.id)).map(m=>[m.id,m]));for(const [id,m]of selected.models)all.set(id,mergeModelSelection(all.get(id),m));
 ...
 $('editor-add').addEventListener('click',()=>{const id=$('editor-id').value.trim();if(!validModelID(id)||s().models.size>=50||ctx.client==='claude-desktop'&&!claudeDesktopModelSupported(id))return;s().models.set(id,{...(ctx.catalog.find(m=>m.id===id)||{id,name:id})});if(!s().initial)s().initial=id;$('editor-search').value='';$('editor-selected').checked=true;$('editor-id').value='';render(ctx)});
 async function loadDesktopLibrary(){
  desktopLibraryLoading=true;working=true;
  const selection=selections['claude-desktop'];
  try{
   const result=await api('model-library');
   if(!selection.models.size){
    const filtered=claudeDesktopSelection(result.library?.models||[],result.library?.defaultModel||'');
    desktopOmitted=filtered.omitted;desktopFallback=filtered.fallback;
    selection.models=new Map(filtered.selection.models.map(m=>[m.id,m]));
    selection.initial=filtered.selection.initial;
   }
