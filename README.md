...
5. Click **Start proxy & go to agents**, choose a project folder, and click **Open Codex** or another installed agent. Supported profiles are prepared automatically from the shared library. Zed also receives its local credential automatically. Cursor and Xcode retain their one-time provider setup under **Options**; Open Design has **Set up connection** on its card.
...
| Open Design | Starts the proxy and opens the installed desktop app; copy local connection fields and exact model IDs for one-time BYOK setup |
...
**Open Design connects to the local proxy through its OpenAI API provider.** Use **Agents → Open Design → Set up connection**, then copy the local URL, local key and exact shared model ID into Open Design once. The launch action starts the proxy and opens an installed macOS or Windows app. Provider settings and later model changes are managed inside Open Design; the helper does not import its settings or synchronize its model library. BYOK has no project file tools. See [Open Design setup and Linux guidance](docs/open-design.md).
